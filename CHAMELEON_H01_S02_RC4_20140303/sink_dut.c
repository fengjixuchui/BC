...
void enterServiceMode( void )
{
    ...
    MessageSend(&theSink.task, EventServiceModeEntered, 0);

        /* Power on immediately */
    MessageSend(&theSink.task, EventPowerOn, 0);
...
void cvcProductionTestEnter ( void )
{
    ...
    ConfigRetrieve(theSink.config_id , PSKEY_FEATURE_BLOCK, buffer, sizeof(feature_config_type)) ; 	
             
    audio_plugin = (buffer[3] >> 8) & 0x7;
    DEBUG(("CVCPT:buffer[3] = [%x]\n", buffer[3]));
    DEBUG(("CVCPT:audio_plugin = [%x]\n", audio_plugin));
    
    a2dp_optional_codecs_enabled = (buffer[5] >> 5) &  0xf;
    DEBUG(("CVCPT:buffer[5] = [%x]\n", buffer[5]));
    DEBUG(("CVCPT:a2dp_optional_codecs_enabled = [%x]\n", a2dp_optional_codecs_enabled));
...
void cvcProductionTestKalimbaMessage ( Task task, MessageId id, Message message )
{
	...
	DEBUG(("CVCPT: CVC: msg id[%x] a[%x] b[%x] c[%x] d[%x]\n", m->id, m->a, m->b, m->c, m->d));
    DEBUG(("CVCPT: test type = %x\n", test_type));
    ...
    switch (test_type)
    {
        ...
        case test_aptx:
        {
            switch (m->id)
            {
                ...
                case 0x100c:
                    DEBUG (("CVCPT: APTX_SECPASSED_MSG\n"));
                    exit(CVC_PRODTEST_PASS);
                    break;
            
                case 0x1013:
                    DEBUG (("CVCPT: APTX_SECFAILED_MSG\n"));
                    exit(CVC_PRODTEST_FAIL);
                    break;
        ...
        default:
        {
            DEBUG (("CVCPT: cvcProductionTestKalimbaMessage unknown test_type\n"));
            break;
        }
    }
...
void enterToneTestMode(void)
{
    ...
    AudioConnect((TaskData *)&csr_dut_audio_plugin, 
                 ...
                 NULL, 
                 &theSink.task);
    AudioPlayTone(sine_tone, theSink.features.QueueEventTones, theSink.codec_task, TonesGetToneVolume(FALSE), features); 
}
