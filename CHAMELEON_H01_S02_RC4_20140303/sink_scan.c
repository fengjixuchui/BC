...
void sinkEnableConnectable( void )
{
    ...
    DEBUG(("MP Enable Connectable %ci\n", theSink.inquiry_scan_enabled ? '+' : '-'));
...
void sinkDisableConnectable( void )
{
    ...
    DEBUG(("MP Disable Connectable %ci\n", theSink.inquiry_scan_enabled ? '+' : '-'));
...
#ifdef ENABLE_GATT
    /*  Have the battery reporter reevaluate its connection mode  */
...
void sinkEnableDiscoverable( void )
{
    ...
    DEBUG(("MP Enable Discoverable %cp\n", theSink.page_scan_enabled ? '+' : '-'));
...
void sinkDisableDiscoverable( void )
{
    ...
    DEBUG(("MP Disable Discoverable %cp\n", theSink.page_scan_enabled ? '+' : '-'));
...
{    
    ...
    if((theSink.MultipointEnable)&&(stateManagerGetState() != deviceLimbo))
    {
       ...
       if(theSink.no_of_profiles_connected < 2)
       {
            DEBUG(("MP Go Conn \n" ));
