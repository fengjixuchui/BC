/****************************************************************************
Copyright (C) Cambridge Silicon Radio Ltd. 2011-2013
...
/* DEVICE_ID_PSKEY allows for PS configurable Device Id information, from
   PSKEY_DEVICE_ID, but requires permanent use of a memory pool to hold the
   SDP record.
...
remote_device CheckRemoteDeviceId (const uint8 *attr_data, uint16 attr_data_size)
{
         ...
         SdpParseGetArbitrary(attr_data_size, attr_data, 0x0205, &vendorIdSrc) )
    {
        ...
        if (PsRetrieve(PSKEY_DEVICE_ID, &device_id, sizeof(device_id)) >= SINK_DEVICE_ID_STRICT_SIZE)
        {
...
void GetDeviceIdFullVersion(uint16 *buffer)
{
    ...
    PsRetrieve(PSKEY_DEVICE_ID, buffer, SINK_DEVICE_ID_SW_VERSION_SIZE);
