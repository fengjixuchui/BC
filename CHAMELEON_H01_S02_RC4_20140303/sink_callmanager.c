...
void sinkInitiateLNR ( hfp_link_priority priority )
{
    ...
    if (!stateManagerIsConnected() )
    {
        ...
        sinkQueueEvent(EventLastNumberRedial) ;
    }
...
#endif        
        MessageSend ( &theSink.task , EventEstablishSLC , 0 ) ;
        sinkQueueEvent( EventInitateVoiceDial ) ;
...
#endif        
        MessageSend ( &theSink.task , EventEstablishSLC , 0 ) ;
        sinkQueueEvent( EventUpdateStoredNumber ) ;
    }
}
...
#endif            
            MessageSend ( &theSink.task , EventEstablishSLC , 0 ) ;
            sinkQueueEvent( EventDialStoredNumber ) ;
        }
    }
    else
	{
        /*The PSKEY could not be read*/
        MessageSend(&theSink.task, EventUpdateStoredNumber, 0);
    }
}
