...
void ButtonsInit ( ButtonsTaskData *pButtonsTask )
{
    ...
    B_DEBUG(("B: capsense %s\n", success ? "OK" : "FAIL: check PSKEY_CAP_SENSE_PRELOAD")) ;
    ...
    switch ( pId )
    {
        ...
        case B_INTERNAL_TIMER:
        {
            ...
            else if ( lBTask->gBTime == B_LONG )
            {
                ...
                MessageSend( &theSink.task , EventVLongTimer , 0 ) ;    
            }
            ...
            else
            {
                ...
                /*notify the app that the timer has expired*/
                MessageSend( &theSink.task , EventLongTimer , 0 ) ;    
...
static void ButtonsLevelDetect ( const uint32 pInput , ButtonsTaskData * pButtonsTask ) 
{
    ...
    /* new input levels processed, store for comparison next time around */
    pButtonsTask->gBOldInputState = lNewInput ;
}
