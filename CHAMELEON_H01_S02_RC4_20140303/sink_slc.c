...
void slcConnectFail(void)
{
    ...
    MessageSend(&theSink.task, EventReconnectFailed, 0);
...
{	    
    ...
    if(ind->status == hfp_disconnect_success || ind->status == hfp_disconnect_link_loss || ind->status == hfp_disconnect_abnormally)
    {
        ...
        MessageSend(&theSink.task , ((ind->status == hfp_disconnect_link_loss) ? EventReconnectFailed : EventSLCDisconnected) , 0) ;
    }
...
static void slcConnectionSetup(hfp_link_priority priority, Sink sink, bdaddr* bd_addr)
{
    ...
    /* Enable +CLIP from AG if using TTS numbers/names or display */
#if defined(TEXT_TO_SPEECH_DIGITS) || \
    defined(TEXT_TO_SPEECH_NAMES) || \
    defined(ENABLE_DISPLAY)
#if !defined(TEXT_TO_SPEECH_NAMES) && !defined(ENABLE_DISPLAY)
    ...
    /* We can get network indications before SLC completes, is this correct?? */
    ...
    MessageSendLater ( &theSink.task , EventCheckForAudioTransfer , 0 , 5000 ) ;
    ...
    if((theSink.features.UseDiffConnectedEventAtPowerOn)&&(theSink.powerup_no_connection))
        MessageSend (&theSink.task , EventSLCConnectedAfterPowerOn , NULL );
    else
        MessageSend (&theSink.task , EventSLCConnected , NULL );    
...
static void slcConnectionComplete(hfp_link_priority priority, Sink sink, bdaddr* bd_addr)
{
    ...
    theSink.no_of_profiles_connected = deviceManagerNumConnectedDevs();
 	SLC_DEBUG(("SLC: Pro Connected[%x], NoOfDev=%x\n", (int)sink,theSink.no_of_profiles_connected)) ;
    ...
    if(theSink.paging_in_progress)
        MessageSend(&theSink.task, EventStopPagingInConnState ,0);
...
bool sinkHandleSlcConnectCfm( const HFP_SLC_CONNECT_CFM_T *cfm )
{
    ...
    if((theSink.MultipointEnable) && (theSink.no_of_profiles_connected == MAX_MULTIPOINT_CONNECTIONS))
    {
        ...
        MessageCancelAll(&theSink.task, EventConnectableTimeout);
       ...
       (stateManagerGetState() != deviceLimbo))
    {
        ...
        else
        {
            ...
            else if(theSink.features.EnterPairingModeOnFailureToConnect && !stateManagerIsConnected())
            {
                ...
                /* enter pairing mode */
                MessageSend(&theSink.task, EventEnterPairing, 0);
...
bool slcGetNextListID(void)
{
    ...
    else
    {
       ...
       {
            if(theSink.NoOfReconnectionAttempts && !theSink.no_of_profiles_connected)
                return TRUE;
            else
                return FALSE;
       }
