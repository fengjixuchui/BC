...
void HandleSdpServiceSearchAttributeCfm (CL_SDP_SERVICE_SEARCH_ATTRIBUTE_CFM_T *cfm)
{
        ...
        {   /* Response obtained, examine it */
            peer_device = CheckRemoteDeviceId(cfm->attributes, cfm->size_attributes);
        }
        
        issueA2dpSignallingConnectResponse(&cfm->bd_addr, peer_device);
}
...
void inquiryResume(void)
{
    if(theSink.inquiry.state == inquiry_idle)
    {
        ...
        if (theSink.inquiry.session == inquiry_session_normal)
        {
            ...
            /* Using GIAC */
            ConnectionInquire(&theSink.task, 0x9E8B33, RSSI_CONF.max_responses, RSSI_CONF.timeout, RSSI_CONF.cod_filter);
        }
        else
        {
            ...
            ConnectionInquire(&theSink.task, 0x9E8B00, 4, 46, AUDIO_MAJOR_SERV_CLASS | AV_MAJOR_DEVICE_CLASS );
        }
    }
}
...
void inquiryStart (bool req_disc)
{
    if(!theSink.inquiry.results)
    {
        ...
        if(RSSI_PAIRING)
        {
            ...
            MessageCancelAll(&theSink.task, EventEstablishSLC);
        ...
        if(theSink.conf1->timeouts.InquiryTimeout_s)
            MessageSendLater(&theSink.task, EventRssiPairTimeout, 0, D_SEC(theSink.conf1->timeouts.InquiryTimeout_s));
    }
}
...
void inquiryStop(void)
{
    ...
    if(theSink.inquiry.results)
    {
        ...
        MessageCancelFirst(&theSink.task, EventRssiResume);
        MessageCancelFirst(&theSink.task, EventRssiPairReminder);
        MessageCancelFirst(&theSink.task, EventRssiPairTimeout);
        MessageCancelFirst(&theSink.task, EventContinueSlcConnectRequest);
        ...
        if(theSink.inquiry.action == rssi_connecting)
            MessageSend(&theSink.task, EventContinueSlcConnectRequest, 0);
...
static void inquiryConnect (uint8 index)
{
    ...
    if(theSink.inquiry.results)
    {
        ...
        {
            theSink.inquiry.state = inquiry_idle;
            MessageSendLater(&theSink.task, EventRssiResume, 0, D_SEC(RSSI_CONF.resume_timeout));
        }
    }
}
...
void inquiryHandleResult( CL_DM_INQUIRE_RESULT_T* result )
{
    ...
    if(theSink.inquiry.results)
    {
        ...
        if(result->status == inquiry_status_result)
        {
            ...
            if ((theSink.inquiry.session == inquiry_session_peer && peer_device) || 
                (theSink.inquiry.session == inquiry_session_normal && !peer_device))
#endif
            {
