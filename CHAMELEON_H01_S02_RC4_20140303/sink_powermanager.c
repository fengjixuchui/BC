...
void powerManagerConfig(const power_config* config)
{
    ...
    if ((theSink.conf1 != NULL) && (config != NULL))
    {
        ...
        {
            if ((theSink.conf1->power.bat_events[level].event + EVENTS_MESSAGE_BASE) == EventCriticalBattery)
            {
...
static void powerManagerHandleVbatNormal(uint8 level)
{
    ...
    MessageSend(&theSink.task, EventOkBattery, 0);
    ...
    if (powerManagerIsChargerConnected())
        MessageSend(&theSink.task, (EventChargerGasGauge0+level), 0);
...
static void powerManagerHandleVbat(voltage_reading vbat, battery_level_source source)
{
    ...
    sinkEvents_t event = EVENTS_MESSAGE_BASE + setting.event;
    ...
    {
        PM_DEBUG(("PM: Sending Event 0x%04X\n", event));
    ...
    switch(event)
    {
        ...
        case EventGasGauge2 :
        case EventGasGauge3 :
            powerManagerHandleVbatNormal(event - EventGasGauge0);
            usbSetVbatDead(FALSE);
            break;
    ...
    if(!theSink.features.ChargerTerminationLEDOveride) 
    {
        ...
        switch(state)
        {
            ...
            case power_charger_boost_external:
                ...
                MessageSend(&theSink.task, EventChargeInProgress, 0);
            break;
            case power_charger_complete:
                PM_DEBUG(("PM: Charge Complete\n"));
                MessageSend(&theSink.task, EventChargeComplete, 0);
            break;
            ...
            case power_charger_disabled:
                ...
                MessageSend(&theSink.task, EventChargeDisabled, 0);
            break;
            default:
...
void handlePowerMessage( Task task, MessageId id, Message message )
{
    switch(id)
    {
        case POWER_INIT_CFM:
        {
            ...
            powerManagerChargerSetup(&cfm->vthm);
        }
        break;
