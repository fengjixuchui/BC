...
static const char * const mic_test_string_res = "OK" ;
...
void sinkHandleUnrecognisedATCmd(HFP_UNRECOGNISED_AT_CMD_IND_T *ind)         
{
    ...
    if ( strncmp(pData, mic_test_string, strlen(mic_test_string)) == 0 ) 
    {
        ...
        AT_DEBUG(("Response %s\n", mic_test_string_res));
        HfpAtCmdRequest(ind->priority, mic_test_string_res);
    }    
...
/****************************************************************************
DESCRIPTION
    Checks for a user event -> AT command match configured in PSKEY_USR_3
    ...
    An EventGasGauge0 or an EventChargerGasGauge0 configured will result in the 
...
void ATCommandPlayEvent ( sinkEvents_t id ) 
{
    ...
    {
        if ( (id-EVENTS_MESSAGE_BASE) == theSink.conf3->gEventATCommands[i].event )     
        {
        ...
        switch ( theSink.conf3->gEventATCommands[i].event + EVENTS_MESSAGE_BASE )
        {
            case EventGasGauge0:
                if ( ( id >= EventGasGauge0 ) && (id <= EventGasGauge3)) 
                {
            ...
            case EventChargerGasGauge0:
                if ( ( id >= EventChargerGasGauge0 ) && ( id <= EventChargerGasGauge3)) 
                {
